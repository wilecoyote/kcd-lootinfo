...
DeerDoe_x = {
  ...
  defaultSoulArchetype = "DeerDoe",
  defaultSoulClass = "doe",
  ...
  Properties = {
    ...
    fileHitDeathReactionsParamsDataFile = "Libs/HitDeathReactionsData/HitDeathReactions_DeerDoe.xml",
    bNotPlayerMountable = 0,

    CharacterSounds =
    {
      footstepEffect = "footsteps_DeerDoe",
      remoteFootstepEffect = "footsteps_DeerDoe",
...
-- JCDLootInfo -------------------------
function DeerDoe_x:OnSave(table)
  table.bSearched = self.bSearched;
end

function DeerDoe_x:OnLoad(table)
  self.bSearched = table.bSearched;
end

function DeerDoe_x:bIsInventoryEmpty()
...
function DeerDoe_x:OnReset(bFromInit, bIsReload)
...
function DeerDoe_x:SetFree(rescuer)
  AI.Signal(SIGNALFILTER_SENDER, 0, "GET_UNTIED", self.id);
end

function DeerDoe_x:OnResetCustom()
  self.isFree = true;
  self:ResetOnUsed();
  -- JCDLootInfo -------------------------
  self.bSearched = 0;
...
function DeerDoe_x:GetUsableMessage()
...
function DeerDoe_x:OnUsed(user)
...
function DeerDoe_x:IsUsable(user)
  if (not user) then
    return 0;
  end
