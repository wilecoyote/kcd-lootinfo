...
RedDeer_x = {
  ...
  defaultSoulArchetype = "RedDeer",
  defaultSoulClass = "deer",
  ...
  Properties = {
    ...
    fileHitDeathReactionsParamsDataFile = "Libs/HitDeathReactionsData/HitDeathReactions_redDeer.xml",
    bNotPlayerMountable = 0,

    CharacterSounds =
    {
      footstepEffect = "footsteps_RedDeer",
      remoteFootstepEffect = "footsteps_RedDeer",
...
-- JCDLootInfo -------------------------
function RedDeer_x:OnSave(table)
  table.bSearched = self.bSearched;
end

function RedDeer_x:OnLoad(table)
  self.bSearched = table.bSearched;
end

function RedDeer_x:bIsInventoryEmpty()
...
function RedDeer_x:OnReset(bFromInit, bIsReload)
...
function RedDeer_x:SetFree(rescuer)
  AI.Signal(SIGNALFILTER_SENDER, 0, "GET_UNTIED", self.id);
end

function RedDeer_x:OnResetCustom()
  self.isFree = true;
  self:ResetOnUsed();
  -- JCDLootInfo -------------------------
  self.bSearched = 0;
...
function RedDeer_x:GetUsableMessage()
...
function RedDeer_x:OnUsed(user)
...
function RedDeer_x:IsUsable(user)
  if (not user) then
    return 0;
  end
