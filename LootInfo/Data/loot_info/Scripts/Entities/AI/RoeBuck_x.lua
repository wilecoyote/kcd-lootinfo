...
RoeBuck_x = {
  ...
  defaultSoulArchetype = "RoeBuck",
  defaultSoulClass = "roe",
  ...
  Properties = {
    ...
    fileHitDeathReactionsParamsDataFile = "Libs/HitDeathReactionsData/HitDeathReactions_Roebuck.xml",
...
-- JCDLootInfo -------------------------
function RoeBuck_x:OnSave(table)
  table.bSearched = self.bSearched;
end

function RoeBuck_x:OnLoad(table)
  self.bSearched = table.bSearched;
end

function RoeBuck_x:bIsInventoryEmpty()
...
function RoeBuck_x:OnReset(bFromInit, bIsReload)
...
function RoeBuck_x:SetFree(rescuer)
  AI.Signal(SIGNALFILTER_SENDER, 0, "GET_UNTIED", self.id);
end

function RoeBuck_x:OnResetCustom()
  self.isFree = true;
  self:ResetOnUsed();
  -- JCDLootInfo -------------------------
  self.bSearched = 0;
...
function RoeBuck_x:GetUsableMessage()
...
function RoeBuck_x:OnUsed(user)
...
function RoeBuck_x:IsUsable(user)
  if (not user) then
    return 0;
  end
